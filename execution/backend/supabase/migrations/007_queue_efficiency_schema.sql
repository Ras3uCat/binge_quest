-- BingeQuest Queue Efficiency Schema Updates (Safe Operations)
-- Adds last_activity_at tracking and efficiency helper functions

-- ============================================
-- UPDATE BADGE CATEGORY CONSTRAINT
-- ============================================

-- Drop the old constraint and add new one with 'activity' category
...
ALTER TABLE public.badges ADD CONSTRAINT badges_category_check
    CHECK (category IN ('genre', 'completion', 'streak', 'milestone', 'activity'));
...
VALUES
    ('Queue Manager', 'Achieve efficiency score of 50+', 'emoji:📊', 'activity', '{"type": "efficiency_score", "value": 50}'),
    ('Efficiency Expert', 'Achieve efficiency score of 75+', 'emoji:📈', 'activity', '{"type": "efficiency_score", "value": 75}'),
    ('Queue Master', 'Achieve efficiency score of 90+', 'emoji:🏆', 'activity', '{"type": "efficiency_score", "value": 90}'),
    ('No Backlog', 'Have zero stale items in your queue', 'emoji:✨', 'activity', '{"type": "stale_items", "value": 0}'),
    ('Clean Sweep', 'Clear 5 stale items in a week', 'emoji:🧹', 'activity', '{"type": "stale_cleared", "value": 5}'),
    ('Consistent Viewer', 'Keep all items active for 2 weeks', 'emoji:⚡', 'activity', '{"type": "all_active_days", "value": 14}'),
    ('Quick Starter', 'Watch 3 items within a day of adding them', 'emoji:🚀', 'activity', '{"type": "same_day_starts", "value": 3}')
